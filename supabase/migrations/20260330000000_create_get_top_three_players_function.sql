-- =====================================================================
-- Migration: Create get_top_three_players_and_matches function
-- Purpose: Return top 3 ranked players and key match data per tournament
--          for tournaments in the last 7 days
-- Date: 2026-03-30
-- =====================================================================

-- -----------------------------------------------------------------------
-- PART 1: Composite type for a single podium entry (one tournament result)
-- -----------------------------------------------------------------------
...
CREATE TYPE nakka.tournament_podium_result AS (
  ...
  -- Each element: { rank, player_id, player_name, player_code,
  --                 score_100_count, score_140_count, score_170_count,
  --                 score_180_count, high_finish, best_leg,
  --                 average_score, first_nine_avg }
  podium_players           JSONB,

  -- Final match JSON (nullable)
  -- { match_id, first_player_name, first_player_code,
  --   second_player_name, second_player_code, match_href,
  --   first_player_stats: {...}, second_player_stats: {...} }
  final_match              JSONB,

  -- Semi-final matches JSON array (0-2 elements, same shape as final_match)
...
BEGIN
  ...
  -- Step 2: Calculate global top 3 winners (rank 1 players) based on average score
  ...
  -- Step 3: Top 3 ranked players per tournament from tournament_player_stats
  ...
  -- Step 4: Collect codes of top-3 players per tournament for match lookup
  top_player_codes AS (
    SELECT
      tps.tournament_id,
  ...
  -- Step 5: Knockout matches involving at least one top-3 player, ranked latest-first.
  ...
  -- Step 6: Keep only the top 3 knockout matches (final + 2 semi-finals)
  key_matches AS (
    SELECT * FROM key_matches_ranked WHERE match_rank <= 3
  ),

  -- Step 7: Aggregate per tournament — rank 1 = final, ranks 2-3 = semi-finals
  ...
  -- Step 8: Identify tournaments won by global top 3 winners
  ...
  -- Step 9: Winner's (rank 1) average score per tournament for intra-day ordering
  winner_avg AS (
    SELECT
      tps.tournament_id,
...
'Returns podium data (top 3 players + key match details) for all tournaments
 from the last 8 days that have completed match imports. Includes final,
 semi-final and optional third-place match data. Results are ordered with
 tournaments won by the global top 3 winners (based on average score) first,
 sorted among themselves by the winner''s global average score DESC, then
 remaining tournaments grouped by calendar day DESC and within each day
 ordered by the 1st-place winner average score DESC. Accessible to anon users.';
