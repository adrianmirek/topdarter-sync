-- Migration: Fix combined player names handling
-- Date: 2026-02-25
-- Description: Modify get_player_matches_by_nickname to exclude combined player names
--              when searching for individual players, but include them when explicitly
--              searching for the combined name.
--              Combined names can use either '/' or '\' as separator.
--              Example: 
--                - Search "Sebastian Szymkowiak" → excludes "Mateusz Obroszko/Sebastian Szymkowiak"
--                - Search "Mateusz Obroszko/Sebastian Szymkowiak" → includes that combined name
--                - Search "Mateusz Obroszko\Sebastian Szymkowiak" → includes that combined name

-- Drop existing function
DROP FUNCTION IF EXISTS nakka.get_player_matches_by_nickname(TEXT[], INTEGER) CASCADE;

-- Recreate function with improved logic for combined player names
    ...
    WHERE 
      ...
      EXISTS (
        ...
        WHERE (
          ...
          OR
          -- For nicknames with '/' or '\', match normally (including combined names)
          (
...
'Retrieves player matches by nickname(s) from the database.
...
- When searching for combined names (e.g., "Mateusz Obroszko/Sebastian Szymkowiak" or "Mateusz Obroszko\Sebastian Szymkowiak"), includes those combined matches
